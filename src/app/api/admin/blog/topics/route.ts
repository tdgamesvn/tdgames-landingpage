...
IMAGE PROMPT rules:
- SHOW THE ACTUAL THING THE SECTION IS ABOUT. Illustrate it literally: the game art artifact, the production step, the file state being described. A reader who sees only the image must be able to guess the section's subject. Metaphors are BANNED — no stacked paper for "layered cost", no torn edge for "a broken handoff", no vellum, no ink, no blotting paper, no abstract material studies. Those read as stock filler and say nothing about our craft.
- Default render style: hand-painted 2D mobile game art, i.e. the work we actually sell — parallax environment layers, a prop or weapon sheet, a tileset, an environment paint-over, a spell/impact effect frame sequence, an animation keyframe strip, a rig skeleton over a prop. Use a clean technical schematic ONLY when the section describes a workflow, a comparison or a handoff with no visual artifact to show (a five-stage pipeline diagram, a feedback loop, two branching production paths).
- Decide the style ONCE per post and state it in plain words at the start of every prompt ("hand-painted 2D mobile game art of...", "clean technical schematic diagram of..."). All images in one post, cover included, share that one style.
- Every image must show a DIFFERENT concrete subject from the others and from the cover. Same subject twice = one illustration, not two.
- FORBIDDEN: characters, people, faces, creatures, mascots, portraits, anime, hero/knight/warrior, any word naming a living figure. We sell hand-drawn character work — an AI character on our own blog undercuts it. Environments, props, effects, tilesets, rigs and diagrams only.
...
- Example (game art): ![Parallax background layers](ai:hand-painted 2D mobile game art, side-scroller parallax background of a cliff edge and distant ruins separated into three depth layers, casual stylised painterly shading, amber key light and cool rim, near-black sky)
- Example (game art): ![Impact effect frame sequence](ai:hand-painted 2D mobile game art, a row of five frames of a fire impact effect breaking apart into embers and smoke, stylised game VFX shapes, amber and charcoal, near-black background)
- Example (schematic): ![Art production pipeline stages](ai:clean technical schematic diagram of five connected production stages flowing left to right as simple geometric blocks and arrows, thin amber lines on near-black, no text)
